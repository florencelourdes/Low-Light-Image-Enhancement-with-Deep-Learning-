...
---
# Important Note About Training Stability

During the initial training runs, **multiple models (Shallow CNN, Deep CNN, U-Net, and ResUNet)** exhibited **training instability**, including:

- NaN losses and metrics
- Divergence after early epochs
- Inconsistent convergence behavior across models

These issues were not isolated to a single architecture, but instead stemmed from a **shared training configuration**.

### Root Causes

The instability was primarily caused by a combination of:

- mixed precision training (AMP)
- relatively high learning rate
- combined loss function (L1 + Charbonnier + SSIM)
- sensitivity of deeper architectures to optimization settings

---

## Stabilization Strategy (Applied to ALL Models)

To ensure a fair and stable comparison, **all models were retrained using a unified and conservative training configuration**:

- Learning rate: `1e-5`
- Loss function: **L1 only**
- Charbonnier loss: disabled
- SSIM loss: disabled
- Gradient clipping: enabled
- Mixed precision (AMP): disabled for unstable models
- Epochs: `20`
- Batch size: `4`
...
The **Datasets** and **runs** folders are **not included in this repository** because they are too large to store on GitLab.
...
Because these outputs can be several gigabytes, they are **not stored in the GitLab repository**.
